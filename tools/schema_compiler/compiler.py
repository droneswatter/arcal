...
{% endfor %}\
    ...
    void reset() override { value_ = enumNotSet; }
    const std::string& typeName() const override {
        static const std::string kName{"{{ type.name }}"};
        return kName;
    }
    ...
    bool isValid() const { return value_ > enumNotSet && value_ < enumMaxExclusive; }
...
{% endfor %}\
        ...
        if (value_ < 0 || value_ >= enumMaxExclusive) return "invalid";
...
{% endfor %}\
        ...
        for (int i = 0; i < total; ++i) {
            if (name == names[i]) {
                ...
                result.value_ = static_cast<EnumerationItem>(i);
                return result;
            }
        }
    ...
    {{ type.cxx_name }}() : value_(enumNotSet) {}
    {{ type.cxx_name }}(const {{ type.cxx_name }}&) = default;
    {{ type.cxx_name }}& operator=(const {{ type.cxx_name }}&) = default;
    ~{{ type.cxx_name }}() override = default;
...
{% for name, xsd_name, tag in type_entries %}\
    ...
    reg.registerByTag({{ tag }}u, "{{ xsd_name }}");
{% endfor %}\
}
...
void {{ type.cxx_name }}_serialize(const uci::base::Accessor& base, std::string& out) {
    const auto& obj = static_cast<const uci::type::{{ type.cxx_name }}&>(base);
    arcal::externalizer::json::emit_string(obj.toName(), out);
}
...
void {{ type.cxx_name }}_serialize(const uci::base::Accessor& base, std::string& out) {
    const auto& obj = static_cast<const uci::type::{{ type.cxx_name }}&>(base);
    arcal::externalizer::json::emit_value(obj.getValue(), out);
}
void {{ type.cxx_name }}_serialize_fields(const uci::base::Accessor& base, std::string& out, bool& first) {
    (void)first;
    {{ type.cxx_name }}_serialize(base, out);
...
{% if type.base_type and type.base_type not in primitive_types %}\
    arcal::externalizer::json::JsonRegistry::instance().lookup("{{ type.base_type }}").serialize_fields(obj, out, first);
{% endif %}\
{% for choice in type.choices %}\
{% for v in choice.variants %}\
    if (obj.is{{ v.cxx_name }}()) {
        arcal::externalizer::json::emit_key("{{ v.name }}", out, first);
{% if v.type_name in primitive_types %}\
        arcal::externalizer::json::emit_value(obj.get{{ v.cxx_name }}(), out);
{% else %}\
        arcal::externalizer::json::JsonRegistry::instance().lookup("{{ v.type_name }}").serialize(obj.get{{ v.cxx_name }}(), out);
{% endif %}\
    }
{% endfor %}\
{% endfor %}\
{% for field in type.fields %}\
{% if field.list_kind %}\
    arcal::externalizer::json::emit_key("{{ field.name }}", out, first);
    out += '[';
...
{% if field.type_name in primitive_types %}\
        arcal::externalizer::json::emit_value(item, out);
{% else %}\
        arcal::externalizer::json::JsonRegistry::instance().lookup("{{ field.type_name }}").serialize(item, out);
{% endif %}\
      }
    }
    out += ']';
{% elif field.optional %}\
    if (obj.has{{ field.cxx_name }}()) {
        arcal::externalizer::json::emit_key("{{ field.name }}", out, first);
{% if field.type_name in primitive_types %}\
        arcal::externalizer::json::emit_value(obj.get{{ field.cxx_name }}(), out);
{% else %}\
        arcal::externalizer::json::JsonRegistry::instance().lookup("{{ field.type_name }}").serialize(obj.get{{ field.cxx_name }}(), out);
{% endif %}\
    }
{% else %}\
    arcal::externalizer::json::emit_key("{{ field.name }}", out, first);
...
void {{ type.cxx_name }}_serialize(const uci::base::Accessor& base, std::string& out) {
    out += '{';
    bool first = true;
    {{ type.cxx_name }}_serialize_fields_impl(static_cast<const uci::type::{{ type.cxx_name }}&>(base), out, first);
    out += '}';
}
void {{ type.cxx_name }}_serialize_fields(const uci::base::Accessor& base, std::string& out, bool& first) {
    {{ type.cxx_name }}_serialize_fields_impl(static_cast<const uci::type::{{ type.cxx_name }}&>(base), out, first);
...
{% for cxx_type, tag in type_entries %}\
        { {{ tag }}u, []() -> uci::base::Accessor* { return new uci::type::{{ cxx_type }}{}; } },
{% endfor %}\
    };
